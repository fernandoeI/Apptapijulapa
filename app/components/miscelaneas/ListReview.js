...
export default function ListReview(props) {
  const { navigation, idMiscelanea } = props;
  ...
  useFocusEffect(
    useCallback(() => {
      db.collection("reviewsMiscelanea")
        .where("idMiscelanea", "==", idMiscelanea)
        .get()
        .then((response) => {
          const resultReview = [];
  ...
  const eHandle = (e) => {
    if (e.id == "edit") {
      navigation.navigate("AddReviewMiscelanea", {
        idMiscelanea,
        option: e.id,
      });
    } else if (e.id == "delete") {
      ...
      {userLogged ? (
        <View>
          ...
          <Text
            style={styles.verMas}
            onPress={() =>
              navigation.navigate("AllReviewsMiscelanea", { reviews })
            }
          >
            Ver todos
          </Text>

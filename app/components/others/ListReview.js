...
export default function ListReview(props) {
  const { navigation, idOther } = props;
  ...
  useFocusEffect(
    useCallback(() => {
      db.collection("reviewsOther")
        .where("idOther", "==", idOther)
        .get()
        .then((response) => {
          const resultReview = [];
  ...
  const eHandle = (e) => {
    if (e.id == "edit") {
      navigation.navigate("AddReviewOther", {
        idOther,
        option: e.id,
      });
    } else if (e.id == "delete") {
    ...
    <View>
      ...
      {userLogged ? (
        <View>
          ...
          <Text
            style={styles.verMas}
            onPress={() => navigation.navigate("AllReviewsOther", { reviews })}
          >
            Ver todos
          </Text>
        </View>
